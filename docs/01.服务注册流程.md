...
| **一致性** | 最终一致（AP）：本地注册立即生效，集群复制异步；冲突用 lastDirtyTimestamp 解决；新实例对其他客户端可见延迟 = 复制延迟 + 缓存同步(≤30s) + 客户端拉取间隔(≤30s) | replicateToPeers 异步、ResponseCache 双层缓存 |
...
1. **实例启动后迟迟不可被发现**
   - 错误信息：无报错,但其他服务调用不到
   - 原因：① 首次注册延迟 40s ② 注册成功但其他客户端缓存未刷新（服务端只读缓存30s + 客户端拉取间隔30s,最长约100s延迟）
...
flowchart TD
    ...
    Q2 -- 能 --> A3[注册成功,是发现侧问题:<br/>检查调用方的缓存刷新<br/>最长延迟约100s]
