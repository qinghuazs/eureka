...
| **一致性** | 最终一致,多级延迟叠加：注册生效→读写缓存失效(即时)→只读缓存(≤30s)→客户端拉取(≤30s),最坏约 60s+;增量错乱由 hashcode 校验+全量兜底修复 | 见 4.3 状态图 |
...
| `ResponseCacheImpl.getValue()` | ResponseCacheImpl.java:370 | 两级缓存查找链 |
